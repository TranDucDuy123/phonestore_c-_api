...
namespace DoAn2VADT.Areas.Admin.Controllers
{
    ...
    public class ImportController : GlobalController
    {
        ...
        {
            // Tạo model nếu cần thiết, ví dụ thông tin sản phẩm để chọn
            var products = _context.Products.Where(p => p.Effective).ToList(); // Lọc sản phẩm hiệu lực
        ...
        {
            // Tạo ID mới cho ImportDetail
            imp.Id = Guid.NewGuid().ToString();
            imp.Total = imp.PriceIn * imp.Quantity;

            // Lấy thông tin sản phẩm từ cơ sở dữ liệu
            var product = _context.Products.SingleOrDefault(p => p.Id == imp.ProductId);

            if (product != null)
            {
                // Gán thông tin sản phẩm vào ImportDetail
                imp.Product = product;  // Gán đối tượng Product vào ImportDetail
            }

            // Lấy các ImportDetails từ session
            var imports = GetImportSession();

            // Tìm sản phẩm đã có trong session
            var existingImport = imports.FirstOrDefault(p => p.ProductId == imp.ProductId);

            if (existingImport != null)
            {
                // Nếu sản phẩm đã tồn tại, tăng thêm số lượng và cập nhật tổng
                existingImport.Quantity += imp.Quantity;
                existingImport.Total = existingImport.PriceIn * existingImport.Quantity;
            }
            else
            {
            ...
            // Lưu lại các ImportDetails vào session
        ...
        {
            var importSession = GetImportSession();
            ViewBag.ImportSession = importSession;
            return View(importSession);
        }

        [HttpPost]
        public async Task<IActionResult> CreateConfirm()
        {
            var importSessions = GetImportSession();  // Lấy các chi tiết nhập kho từ session
            string importID = Guid.NewGuid().ToString();  // Tạo ID cho bản ghi nhập kho mới
            decimal? total = 0;

            // Lặp qua từng chi tiết nhập kho để tính tổng và lưu vào cơ sở dữ liệu
            foreach (var imp in importSessions)
            {
                imp.ImportId = importID;
                total += imp.Total;
                _context.ImportDetails.Add(imp);  // Thêm chi tiết nhập kho
            }
            string userID = HttpContext.Session.GetString(Const.ADMINIDSESSION);  // Lấy userID từ session
                                                                                  // In ra giá trị của session để kiểm tra
            if (string.IsNullOrEmpty(HttpContext.Session.GetString(Const.ADMINIDSESSION)))
            {
                _notyfService.Error("Không tìm thấy thông tin người tạo.");
            ...
            // Sau khi nhập kho thành công, xóa session
            ClearImportSession();

            return RedirectToAction(nameof(Index));  // Chuyển về danh sách nhập kho
        }
