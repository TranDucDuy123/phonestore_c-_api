...
namespace DoAn2VADT.Controllers
{
    public class HomeController : GlobalController
    {
        ...
        {
            var userSession = HttpContext.Session.GetString(Const.USERSESSION);
            string userName = string.Empty;

            // Nếu session có thông tin người dùng, giải mã chuỗi JSON và lấy tên
            if (!string.IsNullOrEmpty(userSession))
            {
                var customer = JsonConvert.DeserializeObject<Customer>(userSession);
                userName = customer.Name;  // Lấy tên người dùng
            }

            return Json(userName); // Trả về tên người dùng dưới dạng JSON
        }
